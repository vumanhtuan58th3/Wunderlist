...
<?php
    ...
    session_start();
    $editList = $_POST['editListName'];
    $listId = $_POST["idList"];
    // if(isset($_POST['done'])){
    $sql = "UPDATE tb_list SET name='$editList' where tb_list.id=$listId";
    $stmt = $db->prepare($sql);
    $stmt->execute();
    print_r($sql);
...
?>
                        <ul class="chap3" >
                            <li class="one-chap3" name="test" >
                                ...
                                <input class="idList" name="lay1" value="<?php echo $listId ?>" style="width: 25px;">
